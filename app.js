...
const bubbleSort = (arrSalida) => {
  let newArrSalida = [...arrSalida];
  let numIterador = -1;
  for (let i = 0; i < newArrSalida.length; i++) {
    for (let j = 0; j < newArrSalida.length - i - 1; j++) {
      if (numerosTotales.indexOf(newArrSalida[j].numero) > numerosTotales.indexOf(newArrSalida[j + 1].numero)) {
        const lesser = newArrSalida[j + 1];
        newArrSalida[j + 1] = newArrSalida[j];
        newArrSalida[j] = lesser;
        numIterador++;
